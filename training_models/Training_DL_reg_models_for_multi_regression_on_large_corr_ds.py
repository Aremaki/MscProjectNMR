...
# # Training of DL regularized models for Multi-regression on 10000 spectra with correlated concentrations 
...
ckpt_large_corr_multi_reg = CheckpointCallback("../saved_models/concentrations/large_corr_multi_reg")
ckpt_large_corr_single_reg = [CheckpointCallback("../saved_models/concentrations/large_corr_single_reg/metabolite_{}"
...
logs_large_corr_multi_reg = tf.keras.callbacks.CSVLogger("../saved_logs/concentrations/large_corr_multi_reg.csv")
logs_large_corr_single_reg = [tf.keras.callbacks.CSVLogger("../saved_logs/concentrations/large_corr_single_reg/metabolite_{}.csv"
                                             .format(i)) for i in range(48)]
...
large_corr_multi_reg_mlp = get_regularised_bn_dropout_mutli_regressor_mlp(input_shape=(10000,), hidden_units=[4096, 512],
                                                                 l2_reg_coeff=0,
                                                                 dropout_rate=0.2,
                                                                 num_outputs=48)
large_corr_multi_reg_mlp.summary()


# ### III.1.b Compile mutli-output MLP


large_corr_multi_reg_mlp.compile(optimizer=tf.keras.optimizers.Adam(0.0008), loss="mse",
                             metrics=[tf.keras.metrics.RootMeanSquaredError()])


# ### III.1.c Fit mutli-output MLP

large_corr_multi_reg_mlp.fit(large_corr_train_dataset, epochs=1000,
                                          validation_data=large_corr_val_dataset,
                                          callbacks=[ckpt_large_corr_multi_reg, logs_large_corr_multi_reg,
                                                     earlystopping_100])

# ### III.1.e Define single-output MLPs


large_corr_inde_reg_mlps = []

for i in range(48):
    large_corr_inde_reg_mlps.append(get_regularised_bn_dropout_mutli_regressor_mlp(input_shape=(10000,),
                                                                          hidden_units=[4096, 512],
                                                                          l2_reg_coeff=0,
                                                                          dropout_rate=0.2,
                                                                          num_outputs=1))

large_corr_inde_reg_mlps[0].summary()
...
for i in range(48):
    large_corr_inde_reg_mlp = large_corr_inde_reg_mlps[i]
    large_corr_inde_reg_mlp.compile(optimizer=tf.keras.optimizers.Adam(0.0008), loss="mse",
                        metrics=[tf.keras.metrics.RootMeanSquaredError()])
...
for i in range(48):
    large_corr_inde_reg_mlp = large_corr_inde_reg_mlps[i]
    large_corr_inde_reg_mlp.fit(large_corr_train_datasets_single[i], epochs=1000,
                       validation_data=large_corr_val_datasets_single[i],
                       callbacks=[ckpt_large_corr_single_reg[i], logs_large_corr_single_reg[i], earlystopping_100])
